...
export function createMockTreeService<T extends TreeNode>(
  ...
  return {
    ...
    create: async () => { throw new Error("Not implemented for mock service"); },
    update: async () => { throw new Error("Not implemented for mock service"); },
    delete: async () => { throw new Error("Not implemented for mock service"); }
  };
}
...
export function useTreeViewModel<T extends TreeNode, TCreate = any, TUpdate = any>(
  ...
  const [selectedValues, setSelectedValues] = useState<string[]>(config.initialSelectedValues || []);
  ...
  }, [service, pagination.page, pagination.pageSize, searchTerm, config.staticData]);
  ...
  const createItem = useCallback(
    async (data: TCreate) => {
      ...
      try {
        ...
        await listTree();
        toast({
          title: t("toast.created").replace("{itemType}", t(`${config.itemTypeNamePlural || 'items'}`)),
          description: t("toast.createSuccess").replace("{itemName}", config.getItemDisplayName?.(data as any) || 'Item'),
          variant: "success",
        });
      } catch (error) {
        toast({
          title: t("toast.createError").replace("{itemType}", t(`${config.itemTypeNamePlural || 'items'}`)),
          description: t("toast.createError").replace("{itemType}", t(`${config.itemTypeNamePlural || 'items'}`)),
          variant: "destructive",
        });
        throw error;
      }
  ...
  const updateItem = useCallback(
    async (id: string, data: TUpdate) => {
      ...
      try {
        ...
        await listTree();
        toast({
          title: t("toast.updated").replace("{itemType}", t(`${config.itemTypeNamePlural || 'items'}`)),
          description: t("toast.updateSuccess").replace("{itemName}", config.getItemDisplayName?.(data as any) || 'Item'),
          variant: "success",
        });
      } catch (error) {
        toast({
          title: t("toast.updateError").replace("{itemType}", t(`${config.itemTypeNamePlural || 'items'}`)),
          description: t("toast.updateError").replace("{itemType}", t(`${config.itemTypeNamePlural || 'items'}`)),
          variant: "destructive",
        });
        throw error;
      }
    ...
    async (item: T) => {
      ...
      await deleteHook.confirmDelete(
        ...
        },
        {
          ...
          successMessage: t("toast.deleteSuccess").replace("{itemName}", config.getItemDisplayName?.(item) || 'Item'),
          errorMessage: t("toast.deleteError").replace("{itemType}", t(`${config.itemTypeNamePlural || 'items'}`)),
        }
      );
    },
  ...
  const openEdit = useCallback((item: T) => {
    ...
    setParentForNew(parent);
    setFormValues(config.getInitialFormValues?.(item, parent || undefined) || {});
    setModalOpen(true);
  ...
  const onSubmit = useCallback(async () => {
    ...
    if (editing) {
      await updateItem(editing.id, config.updateFormData?.(formValues, editing) || formValues);
    } else {
